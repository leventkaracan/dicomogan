...
class ModulationModule(nn.Module):
    def __init__(self, layernum, attr_vec_dim, content_dim):
        super(ModulationModule, self).__init__()
        self.layernum = layernum
        self.fc = nn.Linear(content_dim, content_dim)
        self.norm = nn.LayerNorm([self.layernum, content_dim], elementwise_affine=False)
        self.attr_vec_dim = attr_vec_dim
        self.gamma_function = nn.Sequential(nn.Linear(content_dim, 512), nn.LayerNorm([512]), nn.LeakyReLU(), nn.Linear(512, content_dim))
        self.beta_function = nn.Sequential(nn.Linear(content_dim, 512), nn.LayerNorm([512]), nn.LeakyReLU(), nn.Linear(512, content_dim))
...
class AttributeMapperModule(nn.Module):
    def __init__(self, attr_vec_dim, layernum, content_dim):
        super(AttributeMapperModule, self).__init__()
        self.layernum = layernum
        self.pixelnorm = PixelNorm()
        self.modulation_module_list = nn.ModuleList([ModulationModule(self.layernum, attr_vec_dim, content_dim) for i in range(5)])
...
class AttributeMapper(nn.Module):
    def __init__(self,
        ...
        if self.use_coarse_mapper: 
            self.coarse_mapping = AttributeMapperModule(self.attr_vec_dim, 4, self.content_dim)
        if self.use_medium_mapper:
            self.medium_mapping = AttributeMapperModule(self.attr_vec_dim, 4, self.content_dim)
        if self.use_fine_mapper:
            self.fine_mapping = AttributeMapperModule(self.attr_vec_dim, 10, self.content_dim)
