...
from model import Encoder, EncoderVideo, Decoder, MappingNetworkVAE, LatentODEfunc
...
if __name__ == '__main__':
	...
	for i, data in enumerate(test_loader):
		...
		for l in range(z_vid_l.size(1)):
                        z_vid_l[:, l] = torch.linspace(-1,1,frame_num)
	...
	for i in range(10):
		...
		for l in range(z_vid_l.size(1)):
			...
			html += '</td>\n'
			html += '<td>\n'
			html += '<img src='+ args.output_root + 'video_'+str(i + 1)+'_vae'+str(l + 1)+'.gif width="128px">'
			html += '</td>\n'
			html += '<td>\n'
			html += '<img src='+ args.output_root + 'video_'+str(i + 1)+'_vae'+str(l + 1)+'.png width="1024px">'
			html += '</td>\n'
			html += '</tr>\n'
