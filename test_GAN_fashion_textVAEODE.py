...
from model import Encoder, EncoderVideo_LatentODE, Decoder, MappingNetworkVAE, LatentODEfunc
from data import VideoDataFashionTest
...
def save_batch_images2(batch, path, folder_name, vidno):
    if not os.path.exists(path + folder_name):
        os.mkdir(path + folder_name)
    bs = batch.size(0)
    for i in range(bs):
       save_image((batch[i].data), path + folder_name + "/" + str(vidno)+'_'+str(i)+".png")
...
if __name__ == '__main__':
	...
	M = MappingNetworkVAE(input_dim=8, fsize=256)
	M.eval()
	...
	test_descriptions_file = args.img_root + "metadata/target_descriptions_fashion.txt"
	...
	for i, data in enumerate(test_loader):
		...
		video_sample_rs = nn.functional.interpolate(video_sample, scale_factor=0.5, mode="bicubic", align_corners=False, recompute_scale_factor=True)
		#print(video_sample.size())
		vid_rs = video_sample_rs.view(frame_num, bs, ch, int(height*0.5),int(width*0.5) )
		vid_rs = vid_rs.permute(1,0,2,3,4)
		
		zs, zd = bVAE_enc.test(vid_rs, ts)
		...
		latentw_orig = M(z_vid[:,vae_cond_dim:])
		fake_orig = G(video_sample_norm, txt_feat_orig, latentw_orig)
		fake_target = G(video_sample_norm, txt_feat_target, latentw_orig)
		save_image(((video_sample.data)), args.output_root + '/video_%d_inp.png' % (i + 1))
		save_batch_images(((video_sample.data) ), args.output_root,'video'+str(i+1))

		
		save_image(((fake_orig.data + 1) * 0.5), args.output_root + 'video_%d_rec.png' % (i + 1))
		save_batch_images(((fake_orig.data + 1) * 0.5), args.output_root,'video'+str(i+1)+'_recon')
		save_batch_images2(((fake_orig.data + 1) * 0.5), args.output_root,'recon', i)
		save_image(((fake_target.data + 1) * 0.5), args.output_root + 'video_%d_tar.png' % (i + 1))
		save_batch_images(((fake_target.data + 1) * 0.5), args.output_root,'video'+str(i+1)+'_edited')
		save_batch_images2(((fake_target.data + 1) * 0.5), args.output_root,'target', i)

		input_frames = [imTrans(((video_sample[ind])).cpu()).convert("RGB") for ind in range(video_sample.size(0))]
		...
		first_frame = video_sample_norm[0]
		...
		for l in range(z_vid_l.size(1)):
			if l >= 4:
                        	z_vid_l[:, l] = torch.linspace(zd[0,l-4].item(),zd[T-1,l-4].item(),T)
			else:
				z_vid_l[:, l] = torch.linspace(-2,2,frame_num)
			latentw_target = M(z_vid_l)
			input_video = first_frame
			#if l>=zs.size(1):
			#	input_video = bVAE_dec(z_vid_l)
			#	input_video = nn.functional.interpolate(input_video, scale_factor=2, mode="bicubic", align_corners=False, recompute_scale_factor=True)
			fake_target = G(input_video * 2 - 1, txt_feat_target, latentw_target)
